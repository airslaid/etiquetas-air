...
export const getProductionDataByOP = async (opNumber: number): Promise<PowerBiLabelData[]> => {
  ...
  // Se ainda houver erro, tratamos para a UI
  if (error) {
    console.error("Supabase query error:", error);
    
    // Tratamento específico para API Key inválida
    if (error.message && (error.message.includes("Invalid API key") || error.message.includes("JWT"))) {
        throw new Error("A Chave de API (Anon Key) no arquivo 'supabaseClient.ts' está inválida ou incorreta. Atualize-a com a chave do painel do Supabase.");
    }

    throw new Error(`Erro no Banco de Dados (${error.code || 'Erro Conexão'}): ${error.message}`);
  }
